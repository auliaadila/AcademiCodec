...
from academicodec.utils import scan_checkpoint, load_checkpoint, save_checkpoint, plot_spectrogram
...
def train(rank, a, h):
    ...
    optim_g = torch.optim.Adam(itertools.chain(generator.parameters(),
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        print(f"### Epoch {epoch + 1}/{a.training_epochs}")
        if rank == 0: # rank == 0 ensures logging only happens once (to avoid multiple print statements in multi-GPU settings).
            start = time.time()
        if h.num_gpus > 1: # If h.num_gpus > 1, it sets the epoch for train_sampler (needed for multi-GPU training)
        ...
        for i, batch in enumerate(train_loader):
            ...
            # loss_fm → Feature-matching loss (ensures audio quality).
            loss_fm_f = feature_loss(fmap_f_r, fmap_f_g)
            loss_fm_s = feature_loss(fmap_s_r, fmap_s_g)
            loss_fm_stft = feature_loss(fmap_stftd_r, fmap_stftd_g)

            # loss_gen → Adversarial loss (fooling the discriminator).
            ...
            if rank == 0:
                ...
                # STDOUT logging: Logs training progress every stdout_interval steps.
                ...
                if steps % a.validation_interval == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            # Ensure matching sizes for comparison
                            i_size = min(y_mel.size(2), y_g_hat_mel.size(2))
                            val_err_tot += F.l1_loss(
                                y_mel[:, :, :i_size],
                                y_g_hat_mel[:, :, :i_size]).item()
                            
                            ## SKIP THIS TO SAVE MEMORY
                            '''
                            # Save spectrogram images for the first few samples
                            if j < 8:
                                real_spectrogram = plot_spectrogram(x[0].cpu().numpy())
                                generated_spectrogram = plot_spectrogram(y_g_hat_mel.squeeze(0).cpu().numpy())

                                # Save spectrogram images as files
                                real_spectrogram.savefig(os.path.join(a.checkpoint_path, f"real_spectrogram_{j}.png"))
                                generated_spectrogram.savefig(os.path.join(a.checkpoint_path, f"generated_spectrogram_{j}.png"))

                                print(f"Saved spectrogram comparison for sample {j}")
                            '''

                    # Calculate average validation error
                    val_err = val_err_tot / (j + 1)

                    print(f"Validation Completed | Avg Mel-Spectrogram Error: {val_err:.4f}")
                    print("--- End of Validation ---\n")

                    generator.train()  # Resume training
